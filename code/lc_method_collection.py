...
class LCMethodCollection:
    ...
    def add_method(self, name: str, method: Union[str, LCMethod]):
        """
    ...
    def add_methods_from_paths(self, method_paths: Dict[str, str]):
        """
...
if __name__ == "__main__":
    ...
    # Save to pickle (recommended for Python-only workflows)
    collection.save_pickle('methods.pkl')

    # Save to HDF5 (recommended for integration with h5ad files)
    collection.save_hdf5('methods.h5')

    # Save to JSON (human-readable, version control friendly)
    collection.save_json('methods.json')

    # Load from any format
    new_collection = LCMethodCollection()
    new_collection.load_pickle('methods.pkl')
